...
//	}
	...
	public void AddUserInfo(String UserID,int studentnumber,String realname,String UserTags,String filepath )
	{
		...
		int i=0;
		try {
			sql="update icpdb.userinfo set `studentNumber`= '"+studentnumber+"', `realName`='"
					+ realname+"', `userTags`='"+UserTags+"' where (`userID`='"+UserID+"');" ;
			statement=connection.createStatement();
			statement.execute(sql);
	...
	public void VerifyUser(String UserID,String VeriTag,String filepath)
	{
		...
		ResultSet resultSet=null;
		try {
			sql="update icpdb.userinfo set `VeriTags`='"+VeriTag+"',`VeriPath`='"+filepath+"'where (`userID`='"+UserID+"'); " ;
			statement=connection.createStatement();
			statement.execute(sql);
